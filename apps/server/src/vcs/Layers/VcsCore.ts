...
import {
  findPreferredPullRequestForRemotes,
...
const makeVcsCore = Effect.gen(function* () {
  ...
  const findLatestJjPr = (
    cwd: string,
    bookmarkName: string,
    remotes: ReadonlyArray<JjRemoteInfo>,
    ...
    findPreferredPullRequestForRemotes({
        ...
        remotes,
        preferredRemoteName: preferredRemoteName ?? "origin",
  ...
  const status: VcsCoreShape["status"] = (input) =>
    Effect.gen(function* () {
      ...
      const [gitStatus, currentBaseBookmarks, bookmarkRows, pushRemote, remotes] = yield* Effect.all(
        [
          ...
          listJjRemotes(runJj, input.cwd, "VcsCore.listJjRemotes"),
        ],
      ...
      const prInfo =
        actionBookmark && supportsCreatePullRequest
          ? yield* findLatestJjPr(input.cwd, actionBookmark, remotes, pushRemote?.name ?? null)
          : null;
      return {
        backend: "jj",
        capabilities,
